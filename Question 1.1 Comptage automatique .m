...
for j = 1:6
 ...
 im = dir(fullfile('images','/*.jpg*'));

 Ia= imread((fullfile('images',im(j).name)));
 ...
 fprintf('La somme totale des pièces de M%d vaut %.2f euros.\n',j, somme);
 figure,
 imagesc(Ia);colormap(gray)
title(['\color{red} Image M', num2str(j), ' contient ', num2str(somme), ' €']);
end
